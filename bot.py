    ...
    if message.channel.id in counting_channels:
        ...
        try:
            ...
            if message.author.id == game['last_user']:
                ...
                fail_msg = await message.channel.send(f"{message.author.mention} at your big age still cant count smfh (can't count twice in a row)")
            ...
            if number == game['count'] + 1:
                ...
                await message.delete()
                fail_msg = await message.channel.send(f"{message.author.mention} at your big age still cant count smfh (wrong number, count restarted)")
                await asyncio.sleep(5)
                await fail_msg.delete()
                # Reset count
                game['count'] = 0
